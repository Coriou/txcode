...
const silentPrompt = process.env.T3_ACP_SILENT_PROMPT === "1";
    ...
    Effect.gen(function* () {
      ...
      if (emitToolCalls) {
        ...
        for (let index = 0; index < permissionRequestCount; index++) {
          ...
          const editPermissionRequest = index === 0 && emitEditPermission;
          const permission = yield* agent.client.requestPermission({
            sessionId: requestedSessionId,
            toolCall: {
              toolCallId: index === 0 ? toolCallId : `${toolCallId}-${index + 1}`,
              title: editPermissionRequest
                ? "Delete obsolete file"
                : (process.env.T3_ACP_PERMISSION_TITLE ?? `\`${command}\``),
              ...(editPermissionRequest ? {} : { kind: "execute" as const }),
              status: "pending",
              ...(editPermissionRequest ? { locations: [{ path: "/tmp/obsolete.ts" }] } : {}),
