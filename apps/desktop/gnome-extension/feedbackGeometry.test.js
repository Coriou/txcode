...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "Tx Code");
  expect(findCaptureDestination([window(99, "Tx Code"), target], 42, "Tx Code")).toBe(target);
  expect(findCaptureDestination([window(99, "Tx Code")], 42, "Tx Code")).toBeUndefined();
