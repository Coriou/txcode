// This file mostly exists because we want dev mode to say "Tx Code (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Tx Code (Dev)" : "Tx Code (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `net.coriou.txcode.dev.${devBundleIdSuffix || "local"}`
  : "net.coriou.txcode";
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Tx Code name.
