...
<header>↑ Drag Tx Code into the list above</header>
<button id="app" draggable="true" aria-label="Drag Tx Code to System Settings, or click to reveal in Finder"><img src="${escapeHtml(icon)}" alt="" draggable="false">Tx Code</button>
</main></body></html>`;
}
...
export class MacPermissionHelper {
  ...
  async show(
    ...
    if (!appIcon) throw new Error("The packaged Tx Code icon is missing.");
