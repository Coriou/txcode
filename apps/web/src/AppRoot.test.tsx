...
// AppRoot's notification observer reaches the app graph (router -> routeTree
// -> route components); stub it so this stays a pure structure test.
vi.mock("./notifications/threadNotificationObserver", () => ({
  installThreadNotifications: () => () => {},
}));
import { AppRoot, ThreadNotificationsHost } from "./AppRoot";
...
describe("AppRoot", () => {
  it("shares the application atom registry with routed UI and renderer-wide desktop hosts", () => {
    ...
    expect(isValidElement(children[4]) && children[4].type).toBe(ThreadNotificationsHost);
  });
});
